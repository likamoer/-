/**
 * Created by xufengchao on 16/8/18.
 * 自定义配置, 有几点说明如下:
...
var config = {
  ...
  styleHash: false, // 是否开启样式Hash(避免样式名称重复)
  ...
  externals: [{
    ...
    var: 'postal',
    package: 'postal',
    path: '/node_modules/postal/lib/postal.min.js',
    chunk: ['index']
  }],
  ...
  assets: {
    ...
    css: [{
      path: '/node_modules/font-awesome/css/font-awesome.min.css',
      chunk: ['index'],
      CDN: false
    }, {
  ...
  devServer: {
    ...
    proxy: [ {
      path: /\/api/,
      config: {
        target: 'http://10.16.66.9:5071',
        // target: 'http://127.0.0.1:5071',
        changeOrigin: true,
        logLevel: 'debug',
        ws: true
      }
  ...
  buildDist: {
    public: '/dist/', // script src 或 link href 链接静态文件时路径前缀
